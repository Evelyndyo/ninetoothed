...
class AscendCTarget(EmitterTarget):
    ...
    def render_module(self, context: ModuleRenderContext) -> str:
        ...
        if context.cooperative_reduction_program:
            kernel_prelude = (
                "\n".join(tensor_casts) + "\n" + body if tensor_casts else body
            )
...
{common.indent_block(body, "            ")}
        ...
        support = _ascendc_support(context)
...
__aicore__ inline float nt_sin(float x) {
    const float NT_TWO_PI = 6.2831853f;
    const float NT_PI = 3.14159265f;
    const float NT_HALF_PI = 1.5707964f;
    float s = x - (float)((int)(x * (1.0f / NT_TWO_PI))) * NT_TWO_PI;
    if (s > NT_PI) { s -= NT_TWO_PI; }
    if (s < -NT_PI) { s += NT_TWO_PI; }
    // Fold into [-pi/2, pi/2] so the truncated series stays accurate:
    // sin(pi - s) = sin(s) and sin(-pi - s) = sin(s).
    if (s > NT_HALF_PI) { s = NT_PI - s; }
    else if (s < -NT_HALF_PI) { s = -NT_PI - s; }
    float s2 = s * s;
    return s * (1.0f - s2 * (0.16666667f - s2 * (0.0083333310f
        - s2 * (1.9841270e-4f - s2 * 2.7557314e-6f))));
}
...
__aicore__ inline float nt_pow(float x, float y) {
    ...
    float r = nt_exp(((x < 0.0f) ? nt_floor(y + 0.5f) : y) * lg);
    if (x < 0.0f) {
        float yi = nt_floor(y + 0.5f);
        if (y != yi) { return 0.0f; }
        return (((int)yi) & 1) ? -r : r;
    }
    return r;
...
def _ascendc_support(context: ModuleRenderContext) -> str:
